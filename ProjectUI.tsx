...
export const MosaicItem: React.FC<{ 
  ...
  return (
    <motion.div 
      ...
      // Forceer z-index naar absolute voorgrond bij hover zodat het over alle rijen heen komt
      whileHover={{ 
        zIndex: 100,
        scale: 1.05,
    ...
    >
      ...
      {/* Glow effect op de achtergrond voor diepte als het beeld naar voren komt */}
      ...
      <div className="w-full">
        <div className="h-screen w-full relative">
           ...
           <div className="absolute bottom-24 left-12 md:left-32 max-w-4xl text-white">
              <span className="mono text-xs uppercase tracking-[0.8em] font-black mb-6 block text-amber-500">Masterplan / {project.year}</span>
              ...
              <div className="flex flex-wrap gap-12 md:gap-24 pt-12 border-t border-white/20">
                 <div className="flex flex-col gap-2"><span className="mono text-[10px] uppercase text-white/50 tracking-widest">Locatie</span><span className="text-2xl italic font-serif">{project.location}</span></div>
                 <div className="flex flex-col gap-2"><span className="mono text-[10px] uppercase text-white/50 tracking-widest">Gebruik</span><span className="text-2xl italic font-serif">{project.tag}</span></div>
                 <div className="flex flex-col gap-2"><span className="mono text-[10px] uppercase text-white/50 tracking-widest">Volume</span><span className="text-2xl italic font-serif">{project.area}</span></div>
              </div>
           </div>
        </div>
        <div className="py-64 px-12 md:px-32 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
           <div className="space-y-16">
            ...
            <SafeImage localSrc={imageLocal(project.gallery[1])} fallbackSrc={project.gallery[1]} className="w-full h-full object-cover" alt="Interior detail" />
          </div>
        </div>
        <div className="pb-64 px-12 md:px-32">
           <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                <div className="md:col-span-7"><SafeImage localSrc={imageLocal(project.gallery[2])} fallbackSrc={project.gallery[2]} className="w-full h-[90vh] object-cover shadow-xl" alt="Perspective 1" /></div>
                  <div className="md:col-span-5 pt-32"><SafeImage localSrc={imageLocal(project.gallery[0])} fallbackSrc={project.gallery[0]} className="w-full h-[70vh] object-cover shadow-xl" alt="Perspective 2" /></div>
           </div>
        </div>
